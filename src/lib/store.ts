...
function sanitizePublicArticle(article: Article | null): Article | null {
  if (!article || typeof article.content !== "string") return article;

  const content = article.content
    .replace(
      /> ⚠️ \*\*Answers added by AI\*\*: some answers in this document were generated by AI from general medical knowledge, not the original exam paper\. Verify against your course material before relying on them for the exam\./g,
      "> ⚠️ **Answer note**: some answers in this document may not come from the original exam paper. Verify against your course material before relying on them for the exam."
    )
    .replace(
      /No answer key was present in the source — answer letters previously shown on some questions were AI-generated guesses, not verified answers, and have been removed\. Verify against your course material\./g,
      "No answer key was present in the source. Verify answer keys against your course material before relying on them for the exam."
    );

  return content === article.content ? article : { ...article, content };
}
