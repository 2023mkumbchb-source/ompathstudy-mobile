...
export default function ContestRegistrationPage() {
  ...
  return <div className="min-h-dvh bg-[#071315] px-5 py-10 text-white">
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mt-7 border-y border-white/10 bg-white/[0.025] p-6 sm:p-8">
        ...
        {registration ? <div className="mt-7 rounded-md border border-teal-300/20 bg-teal-300/10 p-5">
          <p className="flex items-center gap-2 font-bold text-teal-200"><CheckCircle2 className="h-5 w-5" /> Contest dashboard</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-md border border-white/10 bg-black/10 p-3"><p className="text-xs text-white/40">University</p><p className="mt-1 font-bold">{registration.contest_universities?.name || "Pending institution review"}</p></div><div className="rounded-md border border-white/10 bg-black/10 p-3"><p className="text-xs text-white/40">Admission</p><p className="mt-1 font-bold capitalize">{registration.status}</p></div></div>
          ...
          <div className="mt-4 flex items-start gap-2 rounded-md border border-white/10 p-3 text-xs leading-relaxed text-white/50"><BellRing className="mt-0.5 h-4 w-4 shrink-0 text-teal-300" /> Site and email reminders follow your notification preference. Calendar reminders remain on your own device.</div>
          {latestResult && resultRound?.results_visible && <div className="mt-4 rounded-md border border-amber-300/20 bg-amber-300/10 p-4"><p className="flex items-center gap-2 font-bold text-amber-200"><Trophy className="h-4 w-4" /> Published result: {latestResult.score}%</p><p className="mt-1 text-xs text-white/50">{resultRound.title} · submitted {latestResult.submitted_at ? new Date(latestResult.submitted_at).toLocaleString("en-KE") : "successfully"}</p><Link to={`/contests/${slug}/certificate/${latestResult.id}`} className="mt-3 inline-flex items-center gap-2 rounded-md border border-amber-200/30 px-3 py-2 text-xs font-bold text-amber-100"><Award className="h-4 w-4" /> Open result certificate</Link></div>}
          <div className="mt-5 flex flex-wrap gap-2"><Link to={`/contests/${slug}/lobby`} className="inline-flex rounded-md border border-teal-200/30 px-4 py-2 text-sm font-bold text-teal-200">Open contest lobby</Link><Link to={`/contests/${slug}/progress`} className="inline-flex rounded-md border border-white/15 px-4 py-2 text-sm font-bold text-white/70">My progression</Link><Link to={`/contests/${slug}/leaderboard`} className="inline-flex rounded-md border border-white/15 px-4 py-2 text-sm font-bold text-white/70">Leaderboard</Link></div>
        </div> : !isOpen ? <div className="mt-7 rounded-md border border-amber-300/20 bg-amber-300/10 p-5">
          ...
          {nextRound?.starts_at && <div className="rounded-md border border-amber-300/20 bg-amber-300/[0.07] p-4"><p className="text-sm font-bold text-amber-200">Scheduled exam</p><ContestCountdown startsAt={nextRound.starts_at} className="mt-3" /><ContestCalendarActions title={`${contest.title} — ${nextRound.title}`} startsAt={nextRound.starts_at} endsAt={nextRound.ends_at} details="Ompath Study medical competition exam. Sign in and enter the secure lobby before the scheduled start." location={`https://www.ompathstudy.com/contests/${slug}/lobby`} className="mt-4" /></div>}
          <label className="block text-sm font-semibold">University<select required value={universityId} onChange={(e) => setUniversityId(e.target.value)} className="mt-2 w-full rounded-md border border-white/15 bg-[#0b1d20] px-3 py-3 text-white"><option value="">Select institution</option>{universities.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}<option value="__new__">My university is not listed</option></select></label>
          {universityId === "__new__" && <div className="rounded-md border border-teal-300/20 bg-teal-300/[0.06] p-4">
            ...
            <label className="mt-4 block text-sm font-semibold">Full university name<input required minLength={3} maxLength={140} value={newUniversityName} onChange={(e) => setNewUniversityName(e.target.value)} placeholder="e.g. Kenya Methodist University" className="mt-2 w-full rounded-md border border-white/15 bg-[#071315] px-3 py-3 text-white placeholder:text-white/25" /></label>
            <label className="mt-4 block text-sm font-semibold">Abbreviation <span className="font-normal text-white/35">(optional)</span><input maxLength={16} value={newUniversityAbbreviation} onChange={(e) => setNewUniversityAbbreviation(e.target.value)} placeholder="e.g. KeMU" className="mt-2 w-full rounded-md border border-white/15 bg-[#071315] px-3 py-3 text-white placeholder:text-white/25" /></label>
          </div>}
          <label className="block text-sm font-semibold">Academic year<select value={studyYear} onChange={(e) => setStudyYear(e.target.value)} className="mt-2 w-full rounded-md border border-white/15 bg-[#0b1d20] px-3 py-3 text-white">{contest.years.map((year) => <option key={year} value={year}>Year {year}</option>)}</select></label>
          ...
          {representation === "university_team" && <div className="grid gap-4 border border-white/10 p-4 sm:grid-cols-2"><label className="block text-sm font-semibold">Team name<input required minLength={2} maxLength={80} value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder="e.g. MKU Haematology Team" className="mt-2 w-full rounded-md border border-white/15 bg-[#0b1d20] px-3 py-3 text-white placeholder:text-white/25" /></label><label className="block text-sm font-semibold">Team role<select value={teamRole} onChange={(e) => setTeamRole(e.target.value as typeof teamRole)} className="mt-2 w-full rounded-md border border-white/15 bg-[#0b1d20] px-3 py-3 text-white"><option value="member">Team member</option><option value="captain">Team captain</option></select></label></div>}
          <div className="border border-white/10 p-4"><p className="flex items-center gap-2 text-sm font-bold"><ShieldCheck className="h-4 w-4 text-teal-300" /> Rules acknowledgement</p><ul className="mt-3 space-y-2 text-xs leading-relaxed text-white/45">{CONTEST_RULES.slice(0,3).map((rule) => <li key={rule}>• {rule}</li>)}</ul><label className="mt-4 flex gap-3 text-sm text-white/65"><input type="checkbox" checked={accepted} onChange={(e) => setAccepted(e.target.checked)} className="mt-1 accent-teal-300" /> I accept the published rules and integrity policy.</label></div>
          {error && <p className="text-sm text-red-300">{error}</p>}
          <button disabled={saving || !accepted || !universityId || (universityId === "__new__" && newUniversityName.trim().length < 3) || (representation === "university_team" && teamName.trim().length < 2)} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-md bg-teal-300 px-5 py-3.5 font-bold text-[#071315] disabled:opacity-40">{saving && <Loader2 className="h-4 w-4 animate-spin" />} Submit registration</button>
        </form>}
      </div>
    </div>
  </div>;
