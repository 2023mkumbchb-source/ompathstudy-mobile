...
export default function ContestAdmin() {
  ...
  return <div className="mx-auto max-w-5xl px-5 py-10">
    <div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="rounded-xl bg-primary/10 p-3"><Trophy className="h-6 w-6 text-primary" /></div><div><h1 className="font-serif text-3xl font-bold">Contest administration</h1><p className="text-sm text-muted-foreground">Control registration and competition stages.</p></div></div><div className="flex flex-wrap gap-2"><Link to="/admin/contests/appeals" className="border px-4 py-2.5 text-sm font-bold">Appeals</Link><Link to="/admin/contests/rehearsal" className="border px-4 py-2.5 text-sm font-bold">Safe rehearsal</Link><Link to="/admin/contests/operations" className="border px-4 py-2.5 text-sm font-bold">Operations</Link><Link to="/admin/contests/questions" className="border px-4 py-2.5 text-sm font-bold">Advanced JSON</Link><Link to="/admin/contests/setup" className="rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground">Set up contest</Link></div></div>
    {message && <p className="mt-6 border bg-muted p-3 text-sm">{message}</p>}
    <div className="mt-8 space-y-4">{contests.map((contest) => <article key={contest.id} className="border bg-card p-5 shadow-sm">
