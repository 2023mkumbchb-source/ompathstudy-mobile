...
export default function ContestLobby() {
  ...
  return <div className="min-h-dvh bg-[#071315] px-3 py-5 text-white sm:px-5 sm:py-10">
    <div className="mx-auto max-w-4xl">
      ...
      {error && <p className="mt-5 rounded-md border border-red-300/20 bg-red-300/10 p-4 text-sm text-red-200">{error}</p>}
      ...
      <section className="mt-4 grid gap-4 sm:mt-6 md:grid-cols-[0.8fr,1.2fr] md:gap-5">
        ...
        <article className="rounded-md border border-white/10 bg-white/[0.035] p-5 sm:p-6">
          <h2 className="flex items-center gap-2 font-bold"><CalendarClock className="h-5 w-5 text-teal-300" /> Competition rounds</h2>
          <div className="mt-5 space-y-3">
            {rounds.length ? rounds.map((round) => <div key={round.id} className="grid grid-cols-[auto,1fr] gap-3 rounded-md border border-white/10 bg-black/10 p-3 sm:flex sm:flex-wrap sm:items-center sm:gap-4 sm:p-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-md bg-white/5 text-sm font-bold text-white/50">{round.round_number}</span>
          ...
          {!liveRound && scheduledRound?.starts_at && <ContestCountdown startsAt={scheduledRound.starts_at} className="mt-5 rounded-md border border-teal-300/20 bg-teal-300/[0.07] p-4" />}
          {!liveRound && !scheduledRound?.starts_at && <div className="mt-5 rounded-md border border-amber-300/15 bg-amber-300/[0.07] p-4"><p className="text-sm font-bold text-amber-200">Schedule not announced</p><p className="mt-1 text-xs leading-relaxed text-white/45">The moderator has not assigned a start date and time yet.</p></div>}
          {liveRound && verified && <div className="mt-5 rounded-md border border-teal-300/20 bg-teal-300/10 p-4"><p className="font-bold text-teal-200">{liveRound.title} is live</p><p className="mt-1 text-xs text-white/50">Your timer begins when the secure attempt is created.</p><Link to={`/contests/${slug}/round/${liveRound.id}`} className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-md bg-teal-300 px-4 py-2.5 text-sm font-bold text-[#071315] sm:w-auto">Enter live round</Link></div>}
        </article>
      </section>
    </div>
  </div>;
