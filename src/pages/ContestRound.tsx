...
export default function ContestRoundPage() {
  ...
  if (!started) return <div className="flex min-h-dvh items-center justify-center bg-[#071315] px-4 py-10 text-white"><div className="w-full max-w-xl rounded-md border border-teal-300/20 bg-[#0b1d20] p-6 shadow-sm sm:p-10"><Trophy className="h-10 w-10 text-amber-300" /><p className="mt-6 text-xs font-bold uppercase tracking-[0.24em] text-teal-300">Competition exam</p><p className="mt-2 text-lg font-black uppercase tracking-wide text-amber-200">{matchup}</p><h1 className="mt-2 font-serif text-3xl font-bold sm:text-4xl">{round?.title}</h1><div className="mt-7 grid grid-cols-2 gap-3"><ExamStat label="Questions" value={String(questions.length)} /><ExamStat label="Time" value={`${Math.floor((round?.duration_seconds || 0) / 60)} min`} /></div><div className="mt-6 rounded-md border border-white/10 p-4"><p className="text-sm font-bold text-white">Device readiness</p><div className="mt-3 space-y-2">{readiness.map((item) => <p key={item.label} className={`flex items-center gap-2 text-xs ${item.ready ? "text-emerald-300" : "text-red-300"}`}>{item.ready ? <CheckCircle2 className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}{item.label}</p>)}</div></div><div className="mt-4 rounded-md border border-white/10 p-4 text-sm leading-relaxed text-white/60"><p className="font-bold text-white">Three-strike integrity rule</p><p className="mt-2">The timer starts when you enter. Switching tabs is recorded. Strikes 1 and 2 show a warning; the third tab switch automatically ends the attempt.</p></div>{error && <p className="mt-4 text-sm text-red-300">{error}</p>}<button onClick={() => void enterExam()} disabled={!readyToStart} className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-md bg-teal-300 px-5 font-bold text-[#071315] disabled:opacity-40"><Expand className="h-5 w-5" /> {readyToStart ? "Enter full-screen exam" : "Resolve readiness checks"}</button></div></div>;

  return <main className="min-h-dvh bg-[#071315] text-white">{strikeNotice && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4"><div className="w-full max-w-sm rounded-md border border-amber-300/30 bg-[#102326] p-7 text-center shadow-sm"><ShieldAlert className="mx-auto h-10 w-10 text-amber-300" /><h2 className="mt-4 text-2xl font-bold">Tab-switch warning</h2><p className="mt-3 text-4xl font-black text-amber-200">Strike {strikeNotice.strikes} of {strikeNotice.limit}</p><p className="mt-3 text-sm leading-relaxed text-white/60">Return to the competition. Your third strike will automatically end this attempt.</p><button onClick={() => { setStrikeNotice(null); void document.documentElement.requestFullscreen?.(); }} className="mt-6 min-h-12 w-full rounded-md bg-amber-300 font-bold text-[#071315]">Continue exam</button></div></div>}<header className="sticky top-0 z-30 border-b border-white/10 bg-[#081719]/95 px-3 py-3 shadow-sm backdrop-blur sm:px-6"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-black uppercase tracking-wider text-amber-200 sm:text-xs">{matchup}</p><p className="truncate text-xs text-white/40">{round?.title} · {answeredCount}/{questions.length} answered</p></div><div className="flex items-center gap-2 sm:gap-4"><div className="text-right"><p className={`flex items-center gap-1.5 font-mono text-xl font-bold ${seconds < 300 ? "text-red-300" : "text-teal-200"}`}><Clock3 className="h-5 w-5" />{time}</p><p className={`text-[10px] font-bold ${online ? "text-emerald-300" : "text-amber-300"}`}>{online ? "Online · autosaving" : "Offline · not saving"}</p></div><button onClick={() => void submitExam()} disabled={finishing || savingIds.size > 0 || !online} className="min-h-11 rounded-md bg-teal-300 px-3 text-xs font-bold text-[#071315] disabled:opacity-40 sm:px-5 sm:text-sm">{finishing ? "Submitting…" : "Submit exam"}</button></div></div></header>
    <div className="mx-auto grid max-w-[1500px] gap-5 px-3 py-5 lg:grid-cols-[240px_minmax(0,1fr)] lg:px-6"><aside className="lg:sticky lg:top-24 lg:h-fit"><div className="rounded-md border border-white/10 bg-[#0b1d20] p-4"><div className="flex items-center justify-between"><p className="text-sm font-bold">Question navigator</p>{!document.fullscreenElement && <button onClick={() => void document.documentElement.requestFullscreen?.()} className="rounded-md border border-white/10 p-2 text-white/60" aria-label="Return to fullscreen"><Expand className="h-4 w-4" /></button>}</div><div className="mt-4 flex gap-2 overflow-x-auto pb-1 lg:grid lg:grid-cols-5">{questions.map((question, index) => <a key={question.id} href={`#question-${question.id}`} className={`flex h-9 min-w-9 items-center justify-center rounded-md border text-xs font-bold ${answers[question.id] !== undefined ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-200" : "border-white/10 text-white/45"}`}>{index + 1}</a>)}</div><p className="mt-4 hidden text-xs leading-relaxed text-white/40 lg:block">All questions are continuous. You may change any answer before submitting.</p></div></aside>
      <section className="space-y-5 pb-28">{error && <div className="rounded-md border border-red-300/20 bg-red-300/10 p-4 text-sm text-red-200">{error}</div>}{questions.map((question, index) => <article id={`question-${question.id}`} key={question.id} onMouseEnter={() => { questionTouchedAt.current[question.id] ||= Date.now(); }} className="scroll-mt-24 rounded-md border border-white/10 bg-[#0b1d20] p-5 shadow-sm sm:p-8"><div className="flex items-start justify-between gap-3"><p className="text-xs font-bold uppercase tracking-wider text-teal-300">Question {index + 1}</p><span className={`flex items-center gap-1 text-[11px] ${savingIds.has(question.id) ? "text-amber-200" : saved[question.id] ? "text-emerald-300" : "text-white/30"}`}>{savingIds.has(question.id) ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3 w-3" />}{savingIds.has(question.id) ? "Saving" : saved[question.id] ? "Saved" : "Not answered"}</span></div><h2 className="mt-3 text-base font-semibold leading-relaxed sm:text-lg">{question.stem}</h2><div className="mt-5 grid gap-3">{question.options.map((option, optionIndex) => <button key={optionIndex} onClick={() => void chooseAnswer(question, optionIndex)} className={`flex min-h-14 w-full items-start gap-3 rounded-md border p-4 text-left text-sm leading-relaxed transition ${answers[question.id] === optionIndex ? "border-teal-300 bg-teal-300/10 text-white ring-1 ring-teal-300/30" : "border-white/10 text-white/65 hover:border-white/25 hover:bg-white/[0.03]"}`}><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-current text-xs font-bold text-teal-300">{String.fromCharCode(65 + optionIndex)}</span><span>{option}</span></button>)}</div></article>)}<div className="rounded-md border border-teal-300/20 bg-teal-300/[0.06] p-6 text-center"><p className="font-bold">End of examination</p><p className="mt-2 text-sm text-white/50">You answered {answeredCount} of {questions.length} questions. Review above or submit when ready.</p><button onClick={() => void submitExam()} disabled={finishing || savingIds.size > 0 || !online} className="mt-5 min-h-12 rounded-md bg-teal-300 px-8 font-bold text-[#071315] disabled:opacity-40">Submit final answers</button></div></section></div>
  </main>;
}

function ExamStat({ label, value }: { label: string; value: string }) { return <div className="rounded-md border border-white/10 bg-white/[0.03] p-4"><p className="text-xs text-white/40">{label}</p><p className="mt-1 text-xl font-bold text-teal-200">{value}</p></div>; }
function StateMessage({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) { return <div className="flex min-h-dvh items-center justify-center bg-[#071315] px-5 text-white"><div className="w-full max-w-md rounded-md border border-white/10 bg-[#0b1d20] p-8 text-center"><div className="flex justify-center">{icon}</div><h1 className="mt-5 font-serif text-3xl font-bold">{title}</h1><p className="mt-3 text-sm leading-relaxed text-white/60">{text}</p></div></div>; }
