...
export default function ContestRehearsal() {
  ...
  return <div className="mx-auto max-w-4xl px-5 py-10"><Link to="/admin/contests" className="inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4" /> Contest administration</Link>
    <header className="mt-7 border-y bg-card p-6"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary"><FlaskConical className="h-4 w-4" /> Isolated rehearsal</p><h1 className="mt-2 font-serif text-3xl font-bold">Run the question experience safely</h1><p className="mt-2 text-sm text-muted-foreground">This scores against the private answer key without creating registrations, attempts, answers, results, or integrity events. It cannot open the production round.</p></header>
    ...
    {error && <p className="mt-5 border border-destructive/30 bg-destructive/10 p-3 text-sm">{error}</p>}
    <div className="mt-7 space-y-5">{questions.map((q, index) => <article key={q.id} className="border bg-card p-5"><p className="text-xs font-bold uppercase text-primary">Question {index + 1}</p><h2 className="mt-2 font-bold">{q.stem}</h2><div className="mt-4 grid gap-2">{q.options.map((option, optionIndex) => <button key={optionIndex} onClick={() => setAnswers((old) => ({ ...old, [q.id]: optionIndex }))} className={`border p-3 text-left text-sm ${answers[q.id] === optionIndex ? "border-primary bg-primary/10" : "hover:bg-muted"}`}>{String.fromCharCode(65 + optionIndex)}. {option}</button>)}</div>{result && <p className={`mt-3 text-sm font-bold ${result.details[index]?.correct ? "text-emerald-600" : "text-red-600"}`}>{result.details[index]?.correct ? "Correct" : "Review needed"}{result.details[index]?.explanation ? ` — ${result.details[index].explanation}` : ""}</p>}</article>)}</div>
    {!questions.length && <p className="mt-8 border border-dashed p-8 text-center text-muted-foreground">No questions are admitted to this round. Import the bank before rehearsing.</p>}
    {questions.length > 0 && <div className="sticky bottom-4 mt-6 flex flex-wrap items-center justify-between gap-3 border bg-background/95 p-4 shadow-lg backdrop-blur"><p className="text-sm">{result ? `${result.correct}/${result.total} correct · ${result.score}%` : `${Object.keys(answers).length}/${questions.length} answered`}</p><button disabled={busy} onClick={() => void score()} className="rounded-lg bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground disabled:opacity-50">Score rehearsal</button></div>}
  </div>;
}
