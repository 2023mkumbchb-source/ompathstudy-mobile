  ...
  return <>
    ...
    <div className="mx-auto max-w-6xl space-y-12 px-5 pb-16">
      {!resourcesLoading && !resourceError && startHere.length > 0 && <section aria-labelledby="start-here-heading">
        ...
        <div className="grid gap-0 divide-y divide-border border-y border-border sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">{startHere.map(({ group, resources, answerReady }) => <div key={group} className="border-y border-border bg-card p-4"><div className="mb-3 flex items-start justify-between gap-2"><h3 className="text-sm font-bold leading-5">{group}</h3><span className="shrink-0 rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-bold text-emerald-700">{answerReady} answer-ready</span></div><div className="space-y-1">{resources.map((resource) => <Link key={resource.id} to={resource.path} className="flex items-start gap-2 rounded-lg p-2 text-sm hover:bg-muted"><BookOpen className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span className="line-clamp-2 font-medium">{resource.title}</span></Link>)}</div></div>)}</div>
      </section>}
