  ...
  return <>
    ...
    <div className="mx-auto max-w-6xl space-y-12 px-5 pb-16">
      {!resourcesLoading && !resourceError && startHere.length > 0 && <section aria-labelledby="start-here-heading">
        ...
        <div className="grid gap-0 divide-y divide-border border-y border-border sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">{startHere.map(({ group, resources, answerReady }) => <div key={group} className="border-y border-border bg-card p-4"><div className="mb-3 flex items-start justify-between gap-2"><h3 className="text-sm font-bold leading-5">{group}</h3><span className="shrink-0 rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-bold text-emerald-700">{answerReady} answer-ready</span></div><div className="space-y-1">{resources.map((resource) => <Link key={resource.id} to={resource.path} className="flex items-start gap-2 border p-2 text-sm hover:bg-muted"><BookOpen className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span className="line-clamp-2 font-medium">{resource.title}</span></Link>)}</div></div>)}</div>
      </section>}
      <section id="all-resources" className="scroll-mt-24">
        <div className="mb-2 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-primary">Complete live library</p><h2 className="font-serif text-2xl font-bold">All revision resources, arranged by unit</h2></div><span className="border border-primary/20 bg-primary/10 px-3 py-1 text-sm font-bold text-primary">{allResources.length} resources</span></div>
        ...
        {withheldCount > 0 && <p className="mb-4 border border-amber-300/50 bg-amber-500/10 p-3 text-sm"><strong>{withheldCount} question resource{withheldCount === 1 ? "" : "s"} held back:</strong> they need answers before they can appear in this revision plan.</p>}
        {!user && <p className="mb-4 border border-amber-300/50 bg-amber-500/10 p-3 text-sm"><Link to="/login" className="font-bold underline">Log in</Link> to sync progress across your phone and other devices. Guest progress stays on this device.</p>}
        <div className="mb-4 grid grid-cols-3 gap-2">
          <button onClick={() => setStatusFilter(statusFilter === "Untouched" ? "All" : "Untouched")} className={`border p-3 text-left ${statusFilter === "Untouched" ? "border-slate-500 bg-slate-500/10" : "bg-card"}`}><span className="block text-xl font-bold">{trackingTotals.untouched}</span><span className="text-[11px] font-semibold text-muted-foreground">Untouched</span></button>
          <button onClick={() => setStatusFilter(statusFilter === "In progress" ? "All" : "In progress")} className={`border p-3 text-left ${statusFilter === "In progress" ? "border-amber-500 bg-amber-500/10" : "bg-card"}`}><span className="block text-xl font-bold">{trackingTotals.inProgress}</span><span className="text-[11px] font-semibold text-muted-foreground">In progress</span></button>
          <button onClick={() => setStatusFilter(statusFilter === "Completed" ? "All" : "Completed")} className={`border p-3 text-left ${statusFilter === "Completed" ? "border-emerald-500 bg-emerald-500/10" : "bg-card"}`}><span className="block text-xl font-bold">{trackingTotals.completed}</span><span className="text-[11px] font-semibold text-muted-foreground">Completed</span></button>
        </div>
      ...
      <section>
        ...
        <div className="mb-4 border border-primary/20 bg-primary/5 p-4 text-sm leading-6"><strong>Daily method:</strong> 60–90 min notes → 45–60 min closed-book questions → 20 min correction log. On mock days, reproduce exam timing and mark every uncertain answer.</div>
        <div className="space-y-2">{plan.map(([date, subject, task]) => {
          const checked = done.includes(date);
          return <button key={date} onClick={() => toggle(date)} className={`grid w-full grid-cols-[2.5rem_4.5rem_minmax(0,1fr)] items-start gap-3 border-y border-border p-3 text-left transition-colors sm:grid-cols-[2.5rem_5rem_9rem_minmax(0,1fr)] ${checked ? "border-primary/30 bg-primary/5" : "bg-card hover:border-primary/30"}`}>
      ...
      <section className="border-y border-foreground bg-foreground p-6 text-background sm:p-8">
