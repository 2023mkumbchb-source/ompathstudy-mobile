...
export default function ContestCertificatePage() {
  ...
  if (!record) return <div className="mx-auto max-w-lg px-5 py-24 text-center"><Award className="mx-auto h-10 w-10 text-muted-foreground" /><h1 className="mt-4 text-2xl font-bold">Certificate unavailable</h1><p className="mt-2 text-muted-foreground">{error}</p><Link to={`/contests/${slug}/register`} className="mt-6 inline-flex border px-4 py-2 font-bold">Return to contest</Link></div>;
  ...
  return <main className="certificate-page min-h-dvh bg-slate-100 px-4 py-8 text-slate-900">
    ...
    <div className="certificate-actions mx-auto mb-5 flex max-w-5xl items-center justify-between gap-3"><Link to={`/contests/${slug}/register`} className="border bg-white px-4 py-2 text-sm font-bold">Back to dashboard</Link><button onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-lg bg-teal-800 px-5 py-2.5 text-sm font-bold text-white"><Printer className="h-4 w-4" /> Download / save as PDF</button></div>
    <section className="certificate-sheet relative mx-auto flex aspect-[1.414/1] max-w-5xl flex-col justify-between overflow-hidden border-[12px] border-teal-900 bg-[#fffdf5] p-8 text-center shadow-lg sm:p-14">
